...
class CredentialStore {
  constructor(dataDir, filename = 'ai-key.dpapi') {
    if (process.platform !== 'win32') throw new Error('此版本仅支持 Windows DPAPI 加密凭据，不会在非 Windows 系统写入明文凭据');
    this.file = path.join(dataDir, 'secrets', filename);
