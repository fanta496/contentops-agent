(() => {
  ...
  const viewNames = {
    ...
    enterprise: ['项目资料库', '管理“做”阶段必须使用的真实项目资料'],
    supervisor: ['运行监控', '查看本地任务状态与提醒'], settings: ['接入与设置', '配置抓取目标、模型、成本与提醒']
  };
  ...
  function bindGlobalActions() {
    ...
    $('#inspectAll').addEventListener('click', async (event) => {
      ...
      if (!result.ok) return toast('巡检失败', result.message || '运行监控未返回有效结果', true);
  ...
  function renderQuickStartGuide() {
    ...
    const steps = [
      ...
      ['4', '生产资料', productionReady, productionReady ? '资料库＋生图已就绪' : !enterpriseExists ? '尚未建立项目资料库' : !enterpriseReady ? '资料库不足，请补充真实品牌、产品或卖点' : '还需配置并测试生图模型']
    ];
    ...
    if (!targetReady) {
      ...
      $('#quickStartDetail').textContent = productionReady ? '点击后会自动开启人工总控，执行抓取和AI分析，到人工选款处暂停。' : '先验证抓取和AI分析；项目资料库与生图连接不会阻止这一轮。';
  ...
  function openVariant(id) {
    ...
    const actualReferenceNotice = !item.imageJob ? '' : item.imageJob.referenceMode !== 'text_only'
      ? imageJobFinished
        ? `最近完成任务实际通过 ${actualProtocolLabel} 传入参考图：${(item.imageJob.referenceAssetIds || []).join('、') || '已传入'}`
        : imageJobActiveReference
          ? `当前任务正在通过 ${actualProtocolLabel} 传入参考图：${(item.imageJob.referenceAssetIds || []).join('、') || '已配置'}`
          : `最近任务配置为 ${actualProtocolLabel}，请结合任务错误确认是否完成传图`
      : imageJobFinished ? `最近完成任务使用${actualProtocolLabel}，未传入参考图` : `当前任务使用${actualProtocolLabel}，不会传入参考图`;
    const policyNotice = referencePolicy === 'disabled' ? '本套明确不使用参考图，将走纯文字生图' : referencePolicy === 'required' ? (!profileSupportsReference ? '本套要求参考图，但当前生图档案不支持；生成会被阻止' : !selectedAssetCount ? '本套要求参考图，但策划未选中可用图片；生成会被阻止' : `本套必须传入 ${selectedAssetCount} 张已选参考图`) : (profileSupportsReference && selectedAssetCount ? `自动模式可用：将传入 ${selectedAssetCount} 张已选参考图` : '自动模式将走纯文字生图（当前档案不支持参考图或本套未选图）');
    const groundingSummary = `<div class="enterprise-grounding"><b>项目依据（本套内容必须围绕它制作）</b><span>角度：${escapeHtml(grounding.productAngle || '未记录')}</span><span>事实：${escapeHtml((grounding.factsUsed || []).join('；') || '未记录')}</span><span>图片：${escapeHtml((grounding.assetUsage || []).join('；') || '未记录')}</span><span>当前判断：${escapeHtml(policyNotice)}</span>${actualReferenceNotice ? `<span>执行记录：${escapeHtml(actualReferenceNotice)}</span>` : ''}</div>`;
    $('#variantModalContent').innerHTML = `<button class="modal-close" id="closeVariantModal" type="button">×</button><form id="variantEditForm" class="studio-workbench"><section class="studio-editor"><div class="studio-context"><span class="section-kicker">一做工作台 / ${escapeHtml(item.platform)}</span><b>参考爆款：${escapeHtml(source?.title || '已选方向')}</b><small>项目资料库：${escapeHtml(activeEnterprise?.name || '未配置')}</small></div>${groundingSummary}<div class="inspector-block"><h4>${escapeHtml(imageJobLabel)}</h4><p class="panel-copy">${escapeHtml(imageJobDetail)}</p></div><label class="editor-field"><span>参考图片使用方式</span><select name="imageReferencePolicy" ${editable && !imageJobActive ? '' : 'disabled'}><option value="auto" ${referencePolicy === 'auto' ? 'selected' : ''}>自动使用（推荐）</option><option value="required" ${referencePolicy === 'required' ? 'selected' : ''}>必须使用参考图</option><option value="disabled" ${referencePolicy === 'disabled' ? 'selected' : ''}>不使用参考图</option></select><small>切换后会废弃旧生成图，防止“图片说用了参考图、实际却没传”的假记录。</small></label><label class="editor-field"><span>标题</span><input name="title" value="${escapeHtml(item.title)}" ${editable && !imageJobActive ? '' : 'disabled'}></label><label class="editor-field"><span>发布正文</span><textarea name="body" rows="12" ${editable && !imageJobActive ? '' : 'disabled'}>${escapeHtml(item.body)}</textarea></label><label class="editor-field"><span>标签（空格或换行分隔）</span><input name="tags" value="${escapeHtml((item.tags || []).map((tag) => `#${tag}`).join(' '))}" ${editable && !imageJobActive ? '' : 'disabled'}></label><div class="studio-toolbar">${editable ? `<button class="secondary-button" id="addImagePage" type="button" ${imageJobActive ? 'disabled' : ''}>+ 添加图片页</button><button class="secondary-button" id="saveVariantEdit" type="submit" ${imageJobActive ? 'disabled' : ''}>保存修改</button><button class="primary-button" id="generateAllImages" type="button" ${imageJobActive ? 'disabled' : ''}>${imageJobActive ? `任务进行中 ${imageJob.completed || 0}/${imageJob.total || 0}` : '生成未完成图片'}</button>${!imageJobActive && imageJob?.status === 'partial' ? '<button class="secondary-button" id="retryFailedImages" type="button">重做未通过图片</button>' : ''}` : ''}</div><div class="studio-pages-editor">${rows}</div></section><aside class="studio-preview"><div class="preview-top"><b>${escapeHtml(item.platform)}图文预览</b><small>实时预览</small></div><div class="xhs-note"><div class="xhs-images" id="xhsPreviewImages"></div><h2 id="xhsPreviewTitle"></h2><p id="xhsPreviewBody"></p><div id="xhsPreviewTags" class="xhs-tags"></div></div><div class="export-box"><b>导出发布包</b><p>审核通过后可导出图片、标题、正文、标签、提示词与来源记录。</p><label class="editor-field"><span>指定导出文件夹</span><input id="variantExportDirectory" value="${escapeHtml(item.exportDirectory || '')}" placeholder="例如 D:\\内容成品"></label>${['approved','exported','published'].includes(item.status) ? '<button class="primary-button" id="exportFromStudio" type="button">导出到指定文件夹</button>' : `<small>当前状态：${statusText(item.status)}。请生成图片、人工审核通过后导出。</small>`}</div></aside></form>`;
  ...
  function renderEnterprise() {
    ...
    $$('.archive-enterprise').forEach((button) => button.addEventListener('click', async () => { if (!confirm('确认停用这份项目资料库吗？历史内容不会删除。')) return; const result = await api.archiveEnterpriseProfile(button.dataset.id); state = await api.getState(); render(); toast(result.ok ? '资料库已停用' : '停用失败', result.message || '', !result.ok); }));
  ...
  function openEnterpriseProfile(id = '') {
    ...
    $('#enterpriseModalTitle').textContent = profile ? '编辑项目资料库' : '新建项目资料库';
    ...
    $('#addEnterpriseImage').onclick = () => {
      if (!form.elements.id.value) return toast('请先保存资料库基本信息', '保存后即可上传参考图片', true);
  ...
  function openEnterpriseExport(profileId) {
    ...
    if (!profile) return toast('导出失败', '未找到项目资料库', true);
  ...
  async function saveEnterpriseImage(event) {
    ...
    try {
      ...
      state = await api.getState(); const profile = state.enterpriseProfiles.find((item) => item.id === payload.profileId); renderEnterpriseImageList(profile?.imageAssets || [], payload.profileId); render(); closeModal('enterpriseImageModal'); toast('参考图片已入库', payload.description ? '已作为可追溯创作约束' : '已本机归档；可在编辑资料中补充图片说明');
  ...
  async function saveEnterpriseProfile(event) {
    ...
    try { const result = await api.saveEnterpriseProfile(data); if (!result.ok) return toast('保存失败', result.message, true); closeModal('enterpriseModal'); state = await api.getState(); render(); toast('项目资料库已保存', result.active ? '已设为当前生产资料库' : '资料已更新'); }
    catch (error) { toast('保存失败', error.message, true); }
  }

  async function saveEnterpriseExport(event) {
    event.preventDefault(); const data = Object.fromEntries(new FormData(event.currentTarget));
    try { const result = await api.exportEnterpriseProfile(data); if (!result.ok) return toast('导出失败', result.message, true); closeModal('enterpriseExportModal'); toast('项目资料库已导出', result.path); }
  ...
  function renderSupervisor() {
    ...
    $('#agentGrid').innerHTML = state.agents.map((agent) => {
      ...
      const detail = enabled ? agent.detail : `${agent.id === 'xhs-collector' ? '小红书' : '抖音'}未启用；不会参与抓取、自动调度或运行提醒。`;
  ...
  function renderSettings() {
    ...
    const webhookInput = form.elements.feishuWebhook;
    if (webhookInput) webhookInput.value = '';
    const clearWebhook = form.elements.clearFeishuWebhook;
    if (clearWebhook) clearWebhook.checked = false;
    const webhookStatus = $('#feishuWebhookStatus');
    if (webhookStatus) webhookStatus.textContent = state.settings.feishuWebhookConfigured ? 'Webhook 已由当前 Windows 用户加密保存' : '尚未配置 Webhook';
  ...
  function renderModelProfiles(kind) {
    ...
    holder.innerHTML = profiles.map((profile) => {
      ...
      const imageCapability = kind === 'image' ? `<br><span class="profile-capability">${profile.imageInputMode === 'reference_generation_json' ? 'Generations JSON 参考图模式，参考图会作为 image 数组传入' : profile.imageInputMode === 'reference_edit' ? 'Edits multipart 参考图模式，参考图会作为文件传入' : '文生图模式，参考图不传入'}</span>` : '';
  ...
  async function saveSettings(event) {
    ...
    data.clearFeishuWebhook = form.elements.clearFeishuWebhook.checked;
